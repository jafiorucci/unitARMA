...
karma.fit<- function (y, ar, ma, a, b, link, names_phi,names_theta,names_beta,diag,h1,X,X_hat,resid)
{
  ...
  if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==T))
  {
    ...
    escore <- function(z)
    {
      ...
      a <- as.vector(a1+a2+a3-(a4*a5)) # derivada em relacao a phi
    ...
    opt <- optim(reg, loglik, escore,
                 ...
                 hessian = TRUE)#, maxit = maxit1, reltol = 1e-12))
  ...
  if(any(is.na(ar)==F) && any(is.na(ma)==F) && any(is.na(X)==F))
  {
    ...
    opt <- optim(reg, loglik, escore,
                 method = "BFGS",
                 #lower = rep(-Inf,length(reg)),
                 #upper = rep(Inf,length(reg)),
                 control = list(fnscale = -1),
                 hessian = TRUE)#, maxit = maxit1, reltol = 1e-12))
...
#' @export
KARMA<- function (y, ar=NA, ma=NA, link="logit",h=6,X=NA,X_hat=NA,resid=3)
{
  #source("karma_media.r")
  #source("karma_fit.r")
  a=0
  b=1
  ...
  fit1 <- karma.fit(y=y, ar=ar, ma=ma, a=a, b=b, link=link1, names_phi=names_phi,
                    names_theta=names_theta, names_beta=names_beta, diag=0, h1=h,
                    X=X, X_hat=X_hat,resid=resid) # model estimation
